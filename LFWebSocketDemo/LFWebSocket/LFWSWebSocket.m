...
#define LFWSSafeCFRelease(obj) if (obj) CFRelease(obj)
...
typedef enum {
    LFWSWebSocketStateNone = 0,
    LFWSWebSocketStateConnecting = 1,
    LFWSWebSocketStateOpen = 2,
    LFWSWebSocketStateClosing = 3,
    LFWSWebSocketStateClosed = 4
}LFWSWebSocketStateType;

@implementation LFWSWebSocket
{
    NSArray *protocols;
    ...
    LFWSMessageProcessor *messageProcessor;
    LFWSFrame *currentFrame;
    uint16_t statusCode;
...
- (id)initWithURL:(NSURL *)url protocols:(NSArray *)protocolStrings {
    ...
    if (self) {
        ...
        messageProcessor = [[LFWSMessageProcessor alloc] init];
...
- (void)closeConnection {
    ...
    inputStream = nil;
    outputStream = nil;
    state = LFWSWebSocketStateClosed;
...
- (BOOL)constructMessage {
    ...
    LFWSMessage *message = [messageProcessor constructMessageFromData:dataReceived];
    
    // Close frame
    if (message.opcode == LFWSWebSocketOpcodeClose) {
        [self sendCloseControlFrameWithStatusCode:message.statusCode text:message.text];
    }
    
    // Ping frame
    if (message.opcode == LFWSWebSocketOpcodePing) {
        LFWSFrame *frame = [[LFWSFrame alloc] initWithOpcode:LFWSWebSocketOpcodePong data:message.data maxSize:fragmentSize];
        [messageProcessor queueFrame:frame];
        [self sendData];
    }
    
    // Pong frame
    if (message.opcode == LFWSWebSocketOpcodePong && pongCallback) {
        dispatch_async(callbackQueue, ^{
            pongCallback();
        });
    }
    ...
    // Text message
    if (message.opcode == LFWSWebSocketOpcodeText && textCallback) {
    ...
    // Binary message
    else if (message.opcode == LFWSWebSocketOpcodeBinary && dataCallback) {
...
- (void)readFromStream {
    ...
    if (state == LFWSWebSocketStateConnecting) {
        [self processResponse];
    }
    
    if (state == LFWSWebSocketStateOpen || state == LFWSWebSocketStateClosing) {
...
- (void)sendCloseControlFrameWithStatusCode:(uint16_t)code text:(NSString *)text {
    ...
    if (state != LFWSWebSocketStateOpen) {
        return;
    }
    
    state = LFWSWebSocketStateClosing;
    ...
    LFWSFrame *frame = [[LFWSFrame alloc] initWithOpcode:LFWSWebSocketOpcodeClose data:frameData maxSize:fragmentSize];
    [messageProcessor queueFrame:frame];
    [self sendData];
}
...
- (BOOL)isValidHandshake:(CFHTTPMessageRef)response {
    ...
    LFWSSafeCFRelease(upgradeValue);
    LFWSSafeCFRelease(connectionValue);
    LFWSSafeCFRelease(acceptValue);
...
- (void)analyzeResponse:(CFHTTPMessageRef)response {
    ...
    else if (state == LFWSWebSocketStateConnecting) {
        [self closeConnection];
    }
}
...
- (void)webSocketThreadLoop {
    @autoreleasepool {
        while (state != LFWSWebSocketStateClosed) {
...
- (void)threadedSendData:(NSData *)data {
    ...
    message.opcode = LFWSWebSocketOpcodeBinary;
    message.data = data;
...
- (void)threadedSendText:(NSString *)text {
    ...
    message.opcode = LFWSWebSocketOpcodeText;
    message.text = text;
...
- (void)threadedSendPingWithData:(NSData *)data {
    if (state == LFWSWebSocketStateConnecting || state == LFWSWebSocketStateOpen) {
        LFWSFrame *frame = [[LFWSFrame alloc] initWithOpcode:LFWSWebSocketOpcodePing data:data maxSize:fragmentSize];
        [messageProcessor queueFrame:frame];
        [self sendData];
    }
}
...
- (void)open {
    if (state != LFWSWebSocketStateNone) {
        return;
    }
    
    state = LFWSWebSocketStateConnecting;
...
- (void)sendRequest:(NSURLRequest *)request {
    NSAssert(state == LFWSWebSocketStateConnecting, @"Requests can only be sent during connecting.");
